...
export interface DocumentUpdate extends StateEvent {
  type: "document-update";
  value: Object;
  changes?: Map<string, Storable | undefined>;
}

export interface DocumentDelete extends StateEvent {
  type: "document-delete";
  key: string;
}

export interface DocumentPatch extends StateEvent {
  type: "document-patch";
  key: string;
  value: PropertyMap;
  remove?: Set<string> | string[];
}

export interface DocumentServiceOpts {
  doc: Y.Doc;
  key: string;
  dispatch?: DocumentEventCallback;
}

export type DocumentEvent = DocumentUpdate | DocumentDelete | DocumentPatch;
export type DocumentEventCallback = (update: DocumentEvent) => void;
export type YMapEventHandler = (event: Y.YMapEvent<Storable>, transaction: Y.Transaction) => void;
export type MessageCallback = (message: any) => void;

export class DocumentService implements StateService {
  key: string;
